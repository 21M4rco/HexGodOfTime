...
with Path('frost-client-smoke.log').open('w') as log:
    try:
        while time.monotonic()<deadline and proc.poll() is None:
            if ready_at is not None and time.monotonic()-ready_at>=8:break
            ...
            if re.search(r'MixinApplyError|Critical injection failure|Exception caught during firing event|Failed to create window',line):
                failed=True;break
            if 'Sound engine started' in line:ready_at=time.monotonic()
    finally:
...
if failed or ready_at is None:sys.exit('Client did not reach the title screen; see frost-client-smoke.log')
print('Client renderer mixins loaded; startup reached the sound engine.')
