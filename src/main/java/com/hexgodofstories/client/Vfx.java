...
public final class Vfx {
    ...
    /** The green wisp trail that marks the keeper's sorcery in flight. */
    public static void trail(Vec3 at,Vec3 velocity,int count) {
        ...
        for(int i=0;i<count;i++) {
            ...
            level.addParticle(HexGodOfStories.EMBER.get(),at.x,at.y,at.z,velocity.x*-.12+jitter.x,velocity.y*-.12+jitter.y,velocity.z*-.12+jitter.z);
        }
    }

    /** One glyph struck open at a point, for the instant a spell resolves. */
    public static void glyph(Vec3 at) {spark(HexGodOfStories.RUNE.get(),at,Vec3.ZERO);}
}
