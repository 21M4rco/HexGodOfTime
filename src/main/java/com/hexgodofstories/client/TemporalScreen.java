...
public final class TemporalScreen {
    ...
    /**
     * The frame a body's camera passes through a Warping break.
     *
     * <p>It has its own curve rather than the shared one, because the shared one opens over five
     * ticks and this has to be at full strength on the frame it starts: the whole job is covering
     * the single moment the client swaps levels, and a ripple that arrives a quarter of a second
     * late covers nothing. Full at the crossing, gone a quarter of a second later.
     */
    public static final int MEMBRANE=7;
    private static PostChain chain;
    private static long start,end;
    private static int mode,width,height;
    private static boolean failed;
    public static void trigger(String effect,boolean self){int duration;switch(effect){case "stop","resume","dilate"->{return;}case "slip"->{mode=2;duration=28;}case "ascend"->{mode=3;duration=self?140:65;}case "fracture","rift_open"->{mode=2;duration=34;}case "rift_cross"->{mode=3;duration=24;}case "warp_membrane"->{mode=MEMBRANE;duration=5;}case "branch_release"->{mode=BRANCH_RELEASE;duration=self?34:22;}default->{return;}}start=ClientState.now();end=start+duration;}
    public static void close(){if(chain!=null)chain.close();chain=null;failed=false;end=0;}
    public static void render(float partial){var mc=Minecraft.getInstance();if(mc.level==null||mc.options.hideGui||failed)return;
        ...
        if(ClientState.now()>end&&sustained<=0)return;
        try{
