...
import com.hexgodofstories.server.RealmShape;
...
public final class RealmAmbience {
    ...
    public static void tick() {
        ...
        for(int i=0;i<(sparse?1:MOTES);i++) {
            ...
            Vfx.spark(HexGodOfStories.MOTE.get(),at,new Vec3((random.nextDouble()-.5)*.006,.004,(random.nextDouble()-.5)*.006));
        }
        ...
        if(random.nextInt(sparse?60:26)==0) {
            ...
            Vfx.spark(HexGodOfStories.NEBULA.get(),at,new Vec3(0,.004,0));
        }
        if(random.nextInt(sparse?90:40)==0)
            Vfx.spark(HexGodOfStories.STAR.get(),eye.add((random.nextDouble()-.5)*24,(random.nextDouble()-.1)*9,(random.nextDouble()-.5)*24),Vec3.ZERO);
    ...
    private static void rim(Minecraft mc,Vec3 eye,long now,boolean sparse,net.minecraft.util.RandomSource random) {
        ...
        for(int i=0;i<(sparse?3:RIM_SAMPLES);i++) {
            ...
            if(random.nextInt(3)==0)Vfx.spark(HexGodOfStories.VEIL.get(),at,outward.add(0,.003,0));
            else Vfx.spark(HexGodOfStories.MOTE.get(),at,outward.add(0,.002,0));
            // An occasional pulse: a brief brighter band, never a constant glow.
            if(random.nextInt(sparse?48:22)==0)
                Vfx.ring(HexGodOfStories.NEBULA.get(),at,.9+random.nextDouble(),3,.01,.006);
        }
    }
}
