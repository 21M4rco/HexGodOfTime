 ...
 *
 * <p>The palette is deliberately not the ultimate's. This is the keeper's ordinary sorcery, so it stays in the
    ...
    public static void tick(long now) {
        ...
        for(var entry:GRASPS.entrySet()) {
            ...
            for(int id:entry.getValue().targets()) {
                ...
                Vfx.gather(HexGodOfStories.EMBER.get(),centre,held.getBbWidth()+.9,Vfx.count(.8f),.07);
                if(now%6==0)Vfx.cloud(HexGodOfStories.NEBULA.get(),centre,held.getBbWidth()*.8+.3,1,.004);
                if(now%8==0)Vfx.spark(HexGodOfStories.GOLD_EMBER.get(),palm(owner,1),Vec3.ZERO);
            }
        }
    }
