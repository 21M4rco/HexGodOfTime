...
public final class Transformation {
    ...
    public static void sustain(ServerPlayer p) {
        ...
        grant(p.getAttribute(Attributes.ARMOR),ARMOUR,"hexgodofstories.mantle.armour",ARMOUR_POINTS);
        grant(p.getAttribute(Attributes.ARMOR_TOUGHNESS),TOUGHNESS,"hexgodofstories.mantle.toughness",TOUGHNESS_POINTS);
        if(HexData.now(p)%RENEW!=0)return;
