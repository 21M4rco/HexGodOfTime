...
final class LegacyRealmGardenV3 {
    ...
    static List<Block> plan() {
        LegacyRealmGardenV3 g=new LegacyRealmGardenV3();
        ...
        for(int i=0;i<19;i++) {
            ...
            end=new Vec3(end.x,LegacyRealmShapeV3.surface((int)end.x,(int)end.z)+.3,end.z);
            ...
            for(int fork:new int[]{-1,1}) {
                ...
                if(!LegacyRealmShapeV3.contains((int)tip.x,(int)tip.z))continue;
                tip=new Vec3(tip.x,LegacyRealmShapeV3.surface((int)tip.x,(int)tip.z)+.3,tip.z);
                g.limb(start,tip,1.3,.2,i+fork,.5);
            }
        }
        // Thick roots curl down over the ragged shore, making the underside part of the tree itself.
        for(int i=0;i<12;i++) {
            double a=i*Math.PI*2/12+.1,r=LegacyRealmShapeV3.edge(a)-3;
            Vec3 lip=new Vec3(50+Math.cos(a)*r,2,50+Math.sin(a)*r/1.07);
            lip=new Vec3(lip.x,LegacyRealmShapeV3.surface((int)lip.x,(int)lip.z)+.2,lip.z);
            Vec3 inner=base.lerp(lip,.62);
            inner=new Vec3(inner.x,LegacyRealmShapeV3.surface((int)inner.x,(int)inner.z),inner.z);
    ...
    private void flowers() {
        ...
        for(int i=0;i<400;i++) {
            ...
            if(!LegacyRealmShapeV3.contains(x,z)||Math.hypot(x-50,z-39)<14||Math.abs(x-50)<7&&z>44&&z<78)continue;
            int y=LegacyRealmShapeV3.surface(x,z)+1;
        ...
        for(int i=0;i<9;i++) {
            ...
            int y=LegacyRealmShapeV3.surface(x,z)+1;BlockPos c=new BlockPos(x,y,z);
    ...
    private void put(BlockPos p,BlockState state) {
        if(p.getY()>-LegacyRealmShapeV3.DEPTH&&p.getY()<150)blocks.put(p,state);
    }
}
