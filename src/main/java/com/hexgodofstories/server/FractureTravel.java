...
import com.hexgodofstories.entity.RiftEntity;
...
public final class FractureTravel {
    ...
    public static FractureMode mode(ServerPlayer owner) {return HexData.fractureMode(owner);}
    ...
    public static void choose(ServerPlayer owner,int index,UUID target) {
        ...
        if(mode.needsTarget) {
            ...
            HexData.fractureTarget(owner,chosen.getUUID(),chosen.getGameProfile().getName());
    ...
    public static boolean act(ServerPlayer owner,int stage) {
        ...
        if(!PocketRealm.inside(owner.level()))return HexServer.pullFracture(owner,stage);
        FractureMode mode=mode(owner);
        // The pull keeps its own tap/hold distinction untouched.
        if(mode==FractureModes.PULL)return HexServer.pullFracture(owner,stage);
        if(stage==RiftEntity.RELEASE)return false;
        FractureAnchor anchor=resolve(owner);
        if(anchor==null)return false;
        return HexServer.openFracture(owner,anchor);
    }
    ...
    private static FractureAnchor towardChosen(ServerPlayer owner) {
        UUID id=HexData.fractureTarget(owner);
    ...
    private static FractureAnchor towardReturn(ServerPlayer owner) {
        CompoundTag d=HexData.get(owner);
