...
/**
 * Every keeper gets a persistent, irregular floating island within their own 512-block-spaced plot.
...
public final class PocketRealm {
    public static final ResourceKey<Level> KEY=ResourceKey.create(Registries.DIMENSION,HexGodOfStories.id("pocket"));
    ...
    private static final String REALMS_FILE="hexgodofstories_realms",LEGACY_REALMS_FILE="loki_realms";
    private static Realms realms(ServerLevel level) {
        var storage=level.getDataStorage();
        return storage.computeIfAbsent(Realms::load,()->{
            // A world saved before the rebrand keeps its plots, their owners and everything built on them.
            Realms carried=storage.get(Realms::load,LEGACY_REALMS_FILE);
            if(carried==null)return new Realms();
            carried.setDirty();
            return carried;
        },REALMS_FILE);
    }
    ...
    public static boolean crossingCooldown(ServerPlayer p) {
        ...
        return !inside(p.level())&&HexData.get(p).getLong("riftGraceUntil")>p.server.overworld().getGameTime();
    }
    ...
    private static CompoundTag travelData(net.minecraft.world.entity.Entity e) {
        if(e instanceof ServerPlayer p)return HexData.get(p);
        CompoundTag parent=e.getPersistentData();
        // A creature carried in before the rebrand keeps the way home it was given under the old name.
        if(!parent.contains(HexData.RETURN_TAG))
    ...
    private static boolean move(ServerPlayer p,ServerLevel destination,Vec3 at,float yaw,float pitch) {
        ...
        long clock=p.server.overworld().getGameTime();
        HexData.get(p).putLong("riftGraceUntil",clock+com.hexgodofstories.entity.RiftEntity.DURATION+20);
        HexData.get(p).putLong("breakGuard",clock+20);
        // An arrival is particles and nothing else: any break this traveller still owns is shut here,
        // in whichever level it was left hanging.
        HexServer.closeRift(p);
        return true;
    }
    ...
    public static boolean sit(ServerPlayer player,BlockPos clicked) {
        ...
        if(!player.level().getEntitiesOfClass(com.hexgodofstories.entity.ThroneSeat.class,new net.minecraft.world.phys.AABB(seatAt,seatAt).inflate(2)).isEmpty())return true;
        var seat=new com.hexgodofstories.entity.ThroneSeat(com.hexgodofstories.HexGodOfStories.THRONE_SEAT.get(),player.level());
