...
public final class Decoy {
    ...
    public static LivingEntity resolve(LivingEntity hunter,LivingEntity wanted) {
        ...
        if(!HexServer.hasProjections(owner.getUUID()))return wanted;
        ...
        pool.add(owner);
        for(IllusionEntity decoy:HexServer.projections(level,owner.getUUID())) {
    ...
    public static void scatter(ServerPlayer owner) {
        if(!(owner.level() instanceof ServerLevel level)||!HexServer.hasProjections(owner.getUUID()))return;
