...
public final class RiftEntity extends Entity {
    ...
    public static RiftEntity open(ServerPlayer p,Vec3 at,boolean homeward,FractureAnchor destination) {
        RiftEntity e=new RiftEntity(HexGodOfStories.RIFT.get(),p.level());
        ...
        p.level().playSound(null,e.blockPosition(),HexGodOfStories.RIFT_OPEN.get(),SoundSource.PLAYERS,.85f,.96f+p.getRandom().nextFloat()*.08f);
        HexNetwork.fx(e,"rift_open");
        return e;
    }
    ...
    @Override public void tick() {
        ...
        for(Entity entity:level().getEntities(this,mouth,this::eligible)) {
            ...
            if(isRemoved())return;
            // The door shuts behind the one who opened it. Nothing of the break is left hanging where
            // they left from, and nothing follows them to where they arrive: only the arrival itself.
            if(crossed&&entity.getUUID().equals(caster)){close();return;}
        }
    }
    ...
    /** Shuts the break for good: the closing sound, its motes, and gone the same tick. */
    public void close() {
        if(isRemoved()||level().isClientSide)return;
        level().playSound(null,blockPosition(),HexGodOfStories.RIFT_CLOSE.get(),SoundSource.PLAYERS,.9f,1);
        HexNetwork.fx(this,"rift_close");
        discard();
    }
    public boolean openedBy(UUID id) {return id!=null&&id.equals(caster);}
