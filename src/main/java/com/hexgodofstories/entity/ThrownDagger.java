...
public final class ThrownDagger extends ThrowableProjectile {
    ...
    public static ThrownDagger throwFrom(ServerPlayer p,Vec3 from,Vec3 aim,boolean illusory) {
        ...
        p.level().playSound(null,e.blockPosition(),HexGodOfStories.BLADE_THROW.get(),SoundSource.PLAYERS,.8f,.95f+p.getRandom().nextFloat()*.12f);
        return e;
    }
    ...
    @Override protected boolean canHitEntity(Entity e) {
        ...
        return flying()&&e!=getOwner()&&(!(getOwner() instanceof ServerPlayer p)||HexServer.validTarget(p,e))&&super.canHitEntity(e);
    }
    ...
    @Override protected void onHitEntity(EntityHitResult hit) {
        ...
        level().playSound(null,blockPosition(),HexGodOfStories.BLADE_HIT.get(),SoundSource.PLAYERS,.9f,1f+random.nextFloat()*.15f);
        HexNetwork.fx(this,"blade_bite",contact.x,contact.y,contact.z);
    ...
    @Override protected void onHitBlock(BlockHitResult hit) {
        ...
        setDeltaMovement(Vec3.ZERO);
        life=140;
        level().playSound(null,blockPosition(),HexGodOfStories.BLADE_EMBED.get(),SoundSource.PLAYERS,.7f,.9f+random.nextFloat()*.2f);
    }

    private void dissolve() {if(!isRemoved()){HexNetwork.fx(this,"dispel");discard();}}
