...
public final class BranchKeyInput {
    ...
    @SubscribeEvent public static void mouse(InputEvent.MouseButton.Post e) {
        if(LokiClient.PRIMARY.matchesMouse(e.getButton()))edge(e.getAction());
    }
    ...
    private static void dispatch(BranchTapGesture.Action action) {
        switch(action) {
            case TAP -> LokiNetwork.send(LokiServer.BRANCH_TAP,0);
            case BEGIN -> LokiNetwork.send(LokiServer.HOLD_BEGIN,0);
            case END -> LokiNetwork.send(LokiServer.HOLD_END,0);
            default -> {}
        }
    }
    static void cancel(boolean release) {
        if(release&&GESTURE.holding())LokiNetwork.send(LokiServer.HOLD_END,0);
        latched=LokiClient.PRIMARY.isDown();owned=false;GESTURE.cancel();
    }
}
