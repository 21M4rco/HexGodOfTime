...
final class ErasureBuffer implements MultiBufferSource {
    ...
    private static final class Vertex {
        ...
        Vertex copy(){Vertex v=new Vertex();v.x=x;v.y=y;v.z=z;v.u=u;v.v=this.v;
            v.nx=nx;v.ny=ny;v.nz=nz;v.r=r;v.g=g;v.b=b;v.a=a;v.lightU=lightU;v.lightV=lightV;return v;}
        static Vertex lerp(Vertex a,Vertex b,float t) {
            Vertex v=new Vertex();float s=1-t;
            v.x=a.x*s+b.x*t;v.y=a.y*s+b.y*t;v.z=a.z*s+b.z*t;
            v.u=a.u*s+b.u*t;v.v=a.v*s+b.v*t;
            v.nx=a.nx*s+b.nx*t;v.ny=a.ny*s+b.ny*t;v.nz=a.nz*s+b.nz*t;
            v.r=Math.round(a.r*s+b.r*t);v.g=Math.round(a.g*s+b.g*t);
            v.b=Math.round(a.b*s+b.b*t);v.a=Math.round(a.a*s+b.a*t);
            v.lightU=Math.round(a.lightU*s+b.lightU*t);v.lightV=Math.round(a.lightV*s+b.lightV*t);return v;
        }
    ...
    private final class FractureVertex implements VertexConsumer {
        ...
        @Override public VertexConsumer vertex(double x,double y,double z) {
            Vector3f p=inverse.transformPosition(new Vector3f((float)x,(float)y,(float)z));
        ...
        @Override public void endVertex() {
            ...
            primitive[count++]=current;current=new Vertex();
        ...
        private void tessellate() {
            ...
            for(int u=0;u<nu;u++)for(int v=0;v<nv;v++) {
                ...
                fragment(new Vertex[]{point(a,b,c,d,u0,v0),point(a,b,c,d,u1,v0),
                    point(a,b,c,d,u1,v1),point(a,b,c,d,u0,v1)});
            }
        }
        private Vertex point(Vertex a,Vertex b,Vertex c,Vertex d,float u,float v) {
            return Vertex.lerp(Vertex.lerp(a,b,u),Vertex.lerp(d,c,u),v);
        }
        private void fragment(Vertex[] vertices) {
            ...
            Vector3f radial=new Vector3f(centre.x,centre.y-height*.5f,centre.z);
            ...
            Vector3f offset=new Vector3f();Quaternionf spin=new Quaternionf();
            if(implosion) {
            ...
            for(Vertex v:vertices) {
                Vector3f p=new Vector3f(v.x,v.y,v.z).sub(centre).mul(scale).rotate(spin).add(centre).add(offset);
                root.transformPosition(p);
                Vector3f normal=new Vector3f(v.nx,v.ny,v.nz).rotate(spin);
