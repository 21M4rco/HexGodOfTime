...
public final class BranchAudio {
    ...
    public static void tick() {
        ...
        for(var player:mc.level.players()) {
            ...
            if(!CHARGES.containsKey(player.getId())) {
                ...
                layers.add(new Layer(player.getId(),Loki.BRANCH_HUM.get(),.62f,.68f,0,1.7f));
                layers.add(new Layer(player.getId(),Loki.BRANCH_RESONANCE.get(),.52f,.82f,0,1.0f));
                layers.add(new Layer(player.getId(),Loki.BRANCH_SHIMMER.get(),.30f,1.24f,BranchCharge.STABLE,.8f));
                layers.add(new Layer(player.getId(),Loki.BRANCH_PRESSURE.get(),.70f,.56f,BranchCharge.PRESSURE,1.0f));
            ...
            if(stage>=2&&ClientState.now()%Math.max(2,9-stage)==0)
                play(Loki.BRANCH_CRACKLE.get(),player.position(),.16f+.05f*stage,.9f+mc.level.random.nextFloat()*.5f);
            ...
            if(held>=BranchCharge.FULL&&ANNOUNCED.add(player.getId()))
                play(Loki.BRANCH_READY.get(),player.position(),.95f,1.28f);
        }
    }
    ...
    public static void discharge(Vec3 at,float power) {
        ...
        play(Loki.BRANCH_OPEN.get(),at,1.25f,.78f-power*.12f);
        Roar roar=new Roar(at,power);
        ROARS.add(roar);
        mc.getSoundManager().play(roar);
    }
    public static void erase(Vec3 at) {play(Loki.BRANCH_ERASE.get(),at,.65f,1.42f);}
    ...
    private static final class Roar extends AbstractTickableSoundInstance {
        ...
        Roar(Vec3 at,float power) {
            super(Loki.BRANCH_ROAR.get(),SoundSource.PLAYERS,SoundInstance.createUnseededRandom());
