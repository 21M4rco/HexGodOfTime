...
public final class LokiAnimations {
   ...
   private static ModifierLayer<IAnimation> layerFor(AbstractClientPlayer player) {
      ...
      if (object instanceof ModifierLayer) {
         ...
         LOGGER.info("Created fallback Loki animation layer for {}", player.getGameProfile().getName());
   ...
   public static void playRemote(int entityId, String name, int fadeTicks) {
      ...
      if (minecraft.level != null && minecraft.level.getEntity(entityId) instanceof AbstractClientPlayer abstractclientplayer) {
         ...
         if ("__clear__".equals(name)) {
            ...
            ResourceLocation resourcelocation = Loki.id(name);
            KeyframeAnimation keyframeanimation = PlayerAnimationRegistry.getAnimation(resourcelocation);
            if (keyframeanimation == null) {
               LOGGER.warn("Loki animation '{}' was requested but not loaded. Expected it under assets/{}/player_animation/.", resourcelocation, "loki");
            } else {
   ...
   public static final class Init {
      ...
      public static void clientSetup(FMLClientSetupEvent event) {
         event.enqueueWork(() -> PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(LokiAnimations.LAYER, 900, player -> new ModifierLayer()));
      }
   }
}
