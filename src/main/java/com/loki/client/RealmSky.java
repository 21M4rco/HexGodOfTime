...
public final class RealmSky extends DimensionSpecialEffects {
    private static VertexBuffer galaxy,spiral;
    ...
    @Override public boolean renderSky(ClientLevel level,int ticks,float partial,PoseStack pose,Camera camera,
        ...
        try {
            ...
            renderGalaxies(pose,projection,ticks+partial);
            celestial(pose,ticks+partial);
        } finally {
            VertexBuffer.unbind();
            pose.popPose();
    ...
    public static void clear(){if(galaxy!=null){galaxy.close();galaxy=null;}if(spiral!=null){spiral.close();spiral=null;}}

    private static void bakeSpiral() {
        ...
        for(int i=0;i<3200;i++) {
            ...
            skyQuad(b,new org.joml.Matrix4f(),new Vec3(x,y,0),new Vec3(size,0,0),new Vec3(0,size,0),red,green,blue,alpha);
        }
        for(int i=20;i>0;i--) {
            double size=i*.24;
            skyQuad(b,new org.joml.Matrix4f(),Vec3.ZERO,new Vec3(size,0,0),new Vec3(0,size,0),.65f,1,.88f,.035f);
        }
        spiral=new VertexBuffer(VertexBuffer.Usage.STATIC);spiral.bind();spiral.upload(b.end());VertexBuffer.unbind();
    }
    private static void renderGalaxies(PoseStack pose,Matrix4f projection,float time) {
        for(int i=0;i<3;i++) {
            pose.pushPose();
            ...
            float scale=i==0?1.15f:.62f;pose.scale(scale,scale,scale);
    ...
    private static void celestial(PoseStack pose,float time) {
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
        BufferBuilder b=Tesselator.getInstance().getBuilder();b.begin(VertexFormat.Mode.QUADS,DefaultVertexFormat.POSITION_COLOR);
        ...
        for(int meteor=0;meteor<6;meteor++) {
            ...
            for(int j=0;j<18;j++) {
                ...
                skyStreak(b,matrix,a,c,.035+(1-t)*.11,.65f,1,.87f,fade*(float)(1-t)*.8f);
            }
            Vec3 right=head.normalize().cross(new Vec3(0,1,0)).normalize();
            Vec3 up=head.normalize().cross(right).normalize();
            skyQuad(b,matrix,head,right.scale(.25),up.scale(.25),.88f,1,.94f,fade);
        }
        ...
        for(int wisp=0;wisp<7;wisp++) {
            ...
            for(int j=0;j<22;j++) {
                ...
                skyStreak(b,matrix,from,to,.06+j*.004,.28f,.9f,.78f,(1-j/22f)*.45f);
            }
            Vec3 right=head.normalize().cross(new Vec3(0,1,0)).normalize(),up=head.normalize().cross(right).normalize();
            skyQuad(b,matrix,head,right.scale(.25),up.scale(.25),.8f,1,.91f,.8f);
        }
        BufferUploader.drawWithShader(b.end());
    }
    private static void skyStreak(BufferBuilder b,Matrix4f m,Vec3 a,Vec3 c,double width,float r,float g,float blue,float alpha) {
        Vec3 along=c.subtract(a).scale(.5),side=along.cross(a).normalize().scale(width);
        skyQuad(b,m,a.lerp(c,.5),along,side,r,g,blue,alpha);
    }
    private static void skyQuad(BufferBuilder b,Matrix4f m,Vec3 centre,Vec3 right,Vec3 up,float r,float g,float blue,float alpha) {
        for(int[] corner:new int[][]{{-1,-1},{1,-1},{1,1},{-1,1}}) {
            Vec3 v=centre.add(right.scale(corner[0])).add(up.scale(corner[1]));
            b.vertex(m,(float)v.x,(float)v.y,(float)v.z).color(r,g,blue,alpha).endVertex();
        }
    }
