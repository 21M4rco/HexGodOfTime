...
public final class IllusionEntity extends PathfinderMob {
    ...
    private void arm(ServerPlayer caster,int loadout) {
        ItemStack main=conjured(loadout==SWORD?Loki.LAEVATEINN.get():Loki.DAGGER.get(),caster,false);
        setItemSlot(EquipmentSlot.MAINHAND,main);
        setItemSlot(EquipmentSlot.OFFHAND,loadout==TWIN?conjured(Loki.DAGGER.get(),caster,true):ItemStack.EMPTY);
    }
    ...
    private void strike(ServerPlayer p,LivingEntity enemy) {
        ...
        float damage=base+Math.min(3.5f,LokiData.mastery(p,Discipline.MISCHIEF)*.0035f);
