...
public final class Telekinesis {
    ...
    private static int capacity(ServerPlayer p) {return 1+LokiData.mastery(p,Discipline.SORCERY)/160;}
    private static double maxDistance(ServerPlayer p) {return 6+LokiData.mastery(p,Discipline.SORCERY)*.016;}
    /**
     ...
     */
    private static double massLimit(ServerPlayer p) {return 4+LokiData.mastery(p,Discipline.SORCERY)*.12;}
    ...
    public static boolean grab(ServerPlayer p,Entity t) {
        ...
        long life=LokiData.now(p)+(t instanceof Player?70:300);
        ...
        p.level().playSound(null,t.blockPosition(),Loki.GRIP_HOLD.get(),SoundSource.PLAYERS,.6f,1.25f);
        sync(p,grip);
        return true;
    }
    ...
    public static void tick(ServerPlayer p) {
        ...
        if(now%14==0)p.level().playSound(null,p.blockPosition(),Loki.GRIP_HOLD.get(),SoundSource.PLAYERS,.24f,1.5f);
    }

    public static void release(ServerPlayer p,boolean thrown) {
        Grip grip=GRIPS.remove(p.getUUID());
        if(grip==null)return;
        double power=2.6+LokiData.mastery(p,Discipline.SORCERY)*.006;
        for(Held h:grip.held) {
            ...
            if(thrown) {
                ...
                if(SLAMS.size()<128)SLAMS.add(new Slam(h.entity,p.getUUID(),power,LokiData.now(p)+50));
