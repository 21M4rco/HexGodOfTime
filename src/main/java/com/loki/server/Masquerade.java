...
public final class Masquerade {
    ...
    public static boolean assume(ServerPlayer p,Entity target) {
        ...
        disguise.putInt("revision",LokiData.get(p).getCompound("disguise").getInt("revision")+1);
        if(!(target instanceof Player))disguise.put("nbt",snapshot(living));
        LokiData.get(p).put("disguise",disguise);
        broadcast(p);
        return true;
    }

    public static void drop(ServerPlayer p) {
        if(!LokiData.get(p).contains("disguise"))return;
        LokiData.get(p).remove("disguise");
        CompoundTag empty=new CompoundTag();
        empty.putBoolean("clear",true);
        LokiNetwork.tracking(p,new LokiNetwork.Message(LokiNetwork.DISGUISE,p.getId(),empty));
        LokiNetwork.fx(p,"disguise");
    }

    /** Fires the heavy payload once, to everyone who can currently see the player. */
    public static void broadcast(ServerPlayer p) {
        CompoundTag disguise=LokiData.get(p).getCompound("disguise");
        if(disguise.isEmpty())return;
        LokiNetwork.tracking(p,new LokiNetwork.Message(LokiNetwork.DISGUISE,p.getId(),disguise.copy()));
    }
    ...
    public static void resend(ServerPlayer viewer,ServerPlayer worn) {
        CompoundTag disguise=LokiData.get(worn).getCompound("disguise");
        if(disguise.isEmpty())return;
        LokiNetwork.to(viewer,new LokiNetwork.Message(LokiNetwork.DISGUISE,worn.getId(),disguise.copy()));
    }
